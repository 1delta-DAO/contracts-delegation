...
- **Morpho / Moolah callbacks missing initiator defense-in-depth** - **Documented as
  intentional** in the codegen templates
  `scripts/_create/templates/flashLoan/morphoCallback.ts` and
  `scripts/_create/templates/flashLoan/moolahCallback.ts` (the per-chain
  `chains/*/flashLoan/callbacks/{Morpho,Moolah}Callback.sol` files are autogenerated
  from these templates and will absorb the doc on next codegen run). Morpho Blue is
  immutable and Lista/Moolah follow the same pattern: both invoke the callback only on
  the `msg.sender` of the originating call, so reaching the handler already implies the
  composer initiated. The `caller() == pool` check is sufficient and is what makes the
  embedded `origCaller` authentic.
