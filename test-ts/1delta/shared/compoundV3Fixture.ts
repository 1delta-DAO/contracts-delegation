...
import { TotalsBasicStructOutput, TotalsCollateralStructOutput } from '../../../types/CometHarness';
...
export type CompoundV3Protocol = {
  opts: ProtocolOpts;
  governor: SignerWithAddress;
  pauseGuardian: SignerWithAddress;
  extensionDelegate: CometExt;
  ...
  comet: Comet;
  tokens: {
    [symbol: string]: FaucetToken & WETH9Mocked;
  };
...
export function defaultAssets(overrides = {}, perAssetOverrides: { [asset: string]: any } = {}): { [asset: string]: any } {
  return {
    COMP: Object.assign({
      initial: 1e7,
      decimals: 18,
...
export async function makeProtocol(opts: ProtocolOpts = {}): Promise<CompoundV3Protocol> {
  const signers = await ethers.getSigners();

  const assets = opts.assets || defaultAssets();
  let priceFeeds: { [asset: string]: any } = {};
  ...
  for (const symbol in assets) {
    ...
    if (symbol !== 'WETH')
      ...
      const WETHFactory = (await ethers.getContractFactory('WETH9Mocked')) as WETH9Mocked__factory;
      token = (tokens[symbol] = await WETHFactory.deploy());
    }
  ...
  const comet = await CometFactory.deploy({
    ...
    baseBorrowMin,
    targetReserves,
    assetConfigs: Object.entries(assets).reduce((acc: any[], [symbol, config], _i) => {
      if (symbol != base) {
        acc.push({
...
export async function makeConfigurator(opts: ProtocolOpts = {}): Promise<ConfiguratorAndProtocol> {
  ...
  const cometProxy = await CometProxy.deploy(
    comet.address,
    proxyAdmin.address,
    String((await comet.populateTransaction.initializeStorage()).data),
  );
  ...
  const initializeCalldata = String((await configurator.populateTransaction.initialize(governor.address)).data);
...
export function objectify(arrayObject: any[]) {
  const obj: any = {};
...
export async function portfolio({ comet, base, tokens }: { comet: any, base: any, tokens: any }, account: string): Promise<Portfolio> {
  const internal: {
    [symbol: string]: bigint;
  } = { [base]: await baseBalanceOf(comet, account) };
  const external: {
    [symbol: string]: bigint;
  } = { [base]: BigInt(await tokens[base].balanceOf(account)) };
...
export async function totalsAndReserves({ comet, base, tokens }: { comet: any, base: any, tokens: any }): Promise<TotalsAndReserves> {
  const totals: {
    [symbol: string]: bigint;
  } = { [base]: BigInt((await comet.totalsBasic()).totalSupplyBase) };
  const reserves: {
    [symbol: string]: bigint;
  } = { [base]: BigInt(await comet.getReserves()) };
  for (const symbol in tokens) {
    if (symbol != base) {
...
export function event(tx: any, index: number) {
  const ev = tx.receipt.events[index], args: any = {};
...
function convertToBigInt(arr: any[]): any[] {
  const newArr = [];
  for (const v of arr) {
    if (Array.isArray(v)) {
