...
export abstract class OrderBase {
    ...
    public async getSignatureWithProviderAsync(
        ...
        const signerObject = await ethers.getSigner(signer)
        switch (type) {
            case SignatureType.EIP712:
                return eip712SignTypedDataWithProviderAsync(this.getEIP712TypedData(), signerObject);
            case SignatureType.EthSign:
                return ethSignHashWithProviderAsync(this.getHash(), signerObject);
            default:
...
export class LimitOrder extends OrderBase {
    ...
    public willExpire(secondsFromNow = 0): boolean {
        ...
        const currentUnixTimestampSec = BigNumber.from(Date.now() / millisecondsInSecond);
...
export class OtcOrder extends OrderBase {
    ...
    public static encodeExpiryAndNonce(expiry: BigNumber, nonceBucket: BigNumber, nonce: BigNumber): BigNumber {
        ...
        return BigNumber.from(
            hexUtils
                .concat(hexUtils.leftPad(expiry.toString(), 8),
