...
async function main() {
    ...
    const txn = DeployFactory__factory.createInterface().encodeFunctionData("deploy", [salt, bytecode])

    const gs = await p.getGasPrice()

    const tx = await operator.sendTransaction({ to: DEPLOY_FACTORY, data: txn, gasLimit: estimate.add(100), gasPrice: gs })
    await tx.wait()
