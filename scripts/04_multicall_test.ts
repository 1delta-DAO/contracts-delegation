...
import UniMulticallAbi from "./abi/UniswapInterfaceMulticall.json"
...
async function main() {
    ...
    const multiContract = await new ethers.Contract(multiAddress, UniMulticallAbi.abi, simpleRpcProvider)

    const x = await multiContract.callStatic.multicall([
        {
            gasLimit: 10_000_000,
    ...
    const y = await multiContract.callStatic.multicall(
        test
    )
