...
abstract contract GenericLending is Slots, ERC20Selectors, Masks {
    ...
    /* layout [underlying,amount,receiver,pool,aToken]
     ...
     * | 96     | 20             | pool                            |
     */
    function _withdrawOrBorrowFromCompoundV3(uint256 currentOffset, address callerAddress, uint256 amountOverride) internal returns (uint256) {
        assembly {
            let ptr := mload(0x40)
            // Aave types need to trasfer collateral tokens
            ...
            let baseToken := shr(96, calldataload(currentOffset))
            ...
            switch and(_PRE_PARAM, amountData)
            case 0 {
                // comet.balanceOf(...) is collateral
                switch eq(underlying, baseToken)
                case 1 {
                    // selector for balanceOf(address)
                    mstore(0, ERC20_BALANCE_OF)
                    // add caller address as parameter
     ...
     * | 76     | 20             | cToken                          |
     */
    function _withdrawOrBorrowFromCompoundV2(uint256 currentOffset, address callerAddress, uint256 amountOverride) internal returns (uint256) {
        assembly {
            let ptr := mload(0x40)
