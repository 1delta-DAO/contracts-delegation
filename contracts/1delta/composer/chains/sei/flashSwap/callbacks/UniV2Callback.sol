...
abstract contract UniV2Callbacks is Masks, DeltaErrors {
    ...
    bytes32 private constant DRAGONSWAP_SEI_V1_FF_FACTORY = 0xff71f6b49ae1558357bBb5A6074f1143c46cBcA03d0000000000000000000000;
    bytes32 private constant DRAGONSWAP_SEI_V1_CODE_HASH = 0x7240a7a71cc85d56ee2e5ca1c5236ce877abc9b49b4696adc94968ef43796fed;
    ...
    function _executeUniV2IfSelector(bytes32 selector) internal {
        ...
        assembly {
            ...
            switch selector
            case 0x99cfb2a400000000000000000000000000000000000000000000000000000000 {
                forkId := and(UINT8_MASK, shr(88, outData))

                ffFactoryAddress := DRAGONSWAP_SEI_V1_FF_FACTORY
                codeHash := DRAGONSWAP_SEI_V1_CODE_HASH
            }
        }
