...
contract BridgeForwarder is StargateV2, Across, Axelar {
    function _bridge(uint256 currentOffset) internal returns (uint256) {
        ...
        } else if (bridgeOperation == BridgeIds.AXELAR) {
            return _bridgeAxelar(currentOffset);
        } else {
            _invalidOperation();
        }
    }
