...
contract BenqiAdapter is ILendingProvider, CTokenSignatures {
    ...
    function repay(ILendingProvider.LendingParams calldata params) external override {
        ...
        (bool success, bytes memory returnData) = params.collateralToken.call(
            abi.encodeWithSelector(CTOKEN_BORROW_BALANCE_CURRENT_SELECTOR, address(this))
        );
        if (!success) {
            assembly {
                revert(add(returnData, 32), mload(returnData))
        ...
        uint256 borrowBalance = abi.decode(returnData, (uint256));

        uint256 amount = params.amount;
        if (params.amount == 0) {
            uint256 assetBalance = IERC20(params.asset).balanceOf(address(this));
            if (assetBalance == 0) revert InsufficientBalance();

            amount = borrowBalance > assetBalance ? assetBalance : borrowBalance;
        } else {
            amount = borrowBalance > params.amount ? params.amount : borrowBalance;
        }

        IERC20(params.asset).approve(params.collateralToken, amount);

        (success, returnData) = params.collateralToken.call(abi.encodeWithSelector(CTOKEN_REPAY_BORROW_SELECTOR, amount));
        if (!success) {
            assembly {
                revert(add(returnData, 32), mload(returnData))
            }
        ...
        uint256 finalAmount = IERC20(params.asset).balanceOf(address(this));
        // refund
        if (finalAmount > 0) {
            IERC20(params.asset).transfer(params.caller, finalAmount);
        }
    }
}
