...
contract OneDeltaQuoter2 {
    ...
    function quoteExactInputV3(bytes memory path, uint256 amountIn) external returns (uint256 amountOut) {
        while (true) {
            bool hasMultiplePools = path.length > 20 + 20 + 3 + 1;

            (address tokenIn, uint8 poolId, uint24 fee, address tokenOut) = decodePool(path);
    ...
    function quoteExactOutputV3(bytes memory path, uint256 amountOut) external returns (uint256 amountIn) {
        while (true) {
            bool hasMultiplePools = path.length > 20 + 20 + 3 + 1;
            (address tokenOut, uint8 poolId, uint24 fee, address tokenIn) = decodePool(path);
    ...
    function quoteExactInput(
        bytes memory path, // calldata more efficient than memory
        uint256 amountIn
    ) public returns (uint256 amountOut) {
        while (true) {
            (address tokenIn, uint8 poolId, address tokenOut) = decodeTokensAndPId(path);

            // v3
            if (poolId < 50) {
                amountIn = quoteExactInputSingleV3(tokenIn, tokenOut, getFee(path), poolId, amountIn);
            }
            // V2
            else if (poolId < 100) {
    ...
    function decodeTokensAndPId(bytes memory data)
        private
        pure
        returns (
            address tokenIn,
    ...
    function decodePool(bytes memory data)
        private
        pure
        returns (
            address tokenIn,
    ...
    function getFee(bytes memory data) private pure returns (uint24 fee) {
        assembly {
            fee := mload(add(add(data, 0x3), 20))
        }
    }

    function skipToken(bytes memory _bytes) private pure returns (bytes memory byteData) {
        uint256 _length = _bytes.length - 24;
        assembly {
            ...
            case 0 {
                ...
                for {
                    let cc := add(add(add(_bytes, lengthmod), mul(0x20, iszero(lengthmod))), 24) // 24 = start
                } lt(mc, end) {
                    mc := add(mc, 0x20)
                    cc := add(cc, 0x20)
                } {
    ...
    function quoteExactOutput(
        ...
        while (true) {
            (address tokenOut, uint8 poolId, address tokenIn) = decodeTokensAndPId(path);

            // v3
            if (poolId < 50) {
                amountOut = quoteExactOutputSingleV3(tokenIn, tokenOut, getFee(path), poolId, amountOut);
            }
            // V2
            else if (poolId < 100) {
                address pair = v2TypePairAddress(tokenIn, tokenOut, poolId);
