...
contract AaveV3FlashLoanCallbackTest is BaseTest, DeltaErrors {
    ...
    function test_unit_lending_flashloans_aaveV3_callback_avalon_skaiaPool() public {
        // mock implementation
        replaceLendingPoolWithMock(AVALON_SKAIA);

        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, AVALON_SKAIA, uint8(2), uint8(56), sweepCall());

        vm.prank(user);
        oneDV2.deltaCompose(params);
    }

    function test_unit_lending_flashloans_aaveV3_callback_rhombusPool() public {
        // mock implementation
        replaceLendingPoolWithMock(RHOMBUS);

        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, RHOMBUS, uint8(2), uint8(86), sweepCall());
    ...
    function getAddressFromRegistry() internal {
        ...
        AVALON_SKAIA = chain.getLendingController(Lenders.AVALON_SKAIA);
        RHOMBUS = chain.getLendingController(Lenders.RHOMBUS);
