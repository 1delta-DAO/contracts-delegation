...
contract BalancerV2FlashLoanCallbackTest is BaseTest, DeltaErrors, Slots {
    ...
    function test_unit_lending_flashloans_balancerV2_callback_swaap_pool_with_callbacks() public {
        // mock implementation
        replaceLendingPoolWithMock(SWAAP);

        bytes memory params = CalldataLib.encodeBalancerV2FlashLoan(USDC, 1e6, uint8(2), sweepCall());
    ...
    function test_unit_lending_flashloans_balancerV2_callback_wrongCaller_revert() public {
        ...
        IVault(validPools[0].poolAddr)
            .flashLoan(address(oneDV2), tokens, amounts, abi.encodePacked(address(user), uint8(validPools[0].poolId)));
    }
