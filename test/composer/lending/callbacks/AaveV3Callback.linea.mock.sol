...
contract AaveV3FlashLoanCallbackTest is BaseTest, DeltaErrors {
    ...
    function test_flash_loan_aaveV3_type_zerolend_pool_with_callbacks() public {
        ...
        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, ZEROLEND, uint8(2), uint8(20), "");

        vm.prank(user);
        oneDV2.deltaCompose(params);
    }

    function test_flash_loan_aaveV3_type_zerolend_croak_pool_with_callbacks() public {
        // mock implementation
        replaceLendingPoolWithMock(ZEROLEND_CROAK);

        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, ZEROLEND_CROAK, uint8(2), uint8(24), "");

        vm.prank(user);
        oneDV2.deltaCompose(params);
    }

    function test_flash_loan_aaveV3_type_zerolend_foxy_pool_with_callbacks() public {
        // mock implementation
        replaceLendingPoolWithMock(ZEROLEND_FOXY);

        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, ZEROLEND_FOXY, uint8(2), uint8(25), "");
