...
contract AaveV2FlashLoanCallbackTest is BaseTest, DeltaErrors {
    ...
    function test_unit_lending_flashloans_aaveV2_callback_lorePool() public {
        replaceLendingPoolWithMock(LORE);

        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, LORE, uint8(3), uint8(8), sweepCall());
    ...
    function test_unit_lending_flashloans_aaveV2_callback_wrongCallerRevert() public {
        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, address(mockPool), uint8(3), uint8(8), sweepCall());
    ...
    function test_unit_lending_flashloans_aaveV2_callback_fuzzInvalidPoolIds(uint8 poolId) public {
        ...
        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, LORE, uint8(3), uint8(poolId), sweepCall());
        vm.prank(user);
    ...
    function getAddressFromRegistry() internal {
        LORE = chain.getLendingController(Lenders.LORE);
    ...
    function populateValidPools() internal {
        validPools.push(PoolCase({poolId: 8, poolAddr: LORE, asset: USDC}));
    }
