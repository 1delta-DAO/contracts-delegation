...
contract AaveV2FlashLoanCallbackTest is BaseTest, DeltaErrors {
    ...
    function test_unit_lending_flashloans_aaveV2_callback_phiatPool() public {

    replaceLendingPoolWithMock(PHIAT);

        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, PHIAT, uint8(3), uint8(16), sweepCall());
    ...
    function test_unit_lending_flashloans_aaveV2_callback_wrongCallerRevert() public {
        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, address(mockPool), uint8(3), uint8(16), sweepCall());
    ...
    function test_unit_lending_flashloans_aaveV2_callback_fuzzInvalidPoolIds(uint8 poolId) public {
        ...
        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, PHIAT, uint8(3), uint8(poolId), sweepCall());
        vm.prank(user);
    ...
    function getAddressFromRegistry() internal {
        PHIAT = chain.getLendingController(Lenders.PHIAT);
    ...
    function populateValidPools() internal {
        validPools.push(PoolCase({poolId: 16, poolAddr: PHIAT, asset: USDC}));
