...
contract AaveV2FlashLoanCallbackTest is BaseTest, DeltaErrors {
    ...
    function setUp() public virtual {
        string memory chainName = Chains.CELO_MAINNET;
    ...
    function test_unit_lending_flashloans_aaveV2_callback_moolaPool() public {

    replaceLendingPoolWithMock(MOOLA);

        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, MOOLA, uint8(3), uint8(14), sweepCall());
    ...
    function test_unit_lending_flashloans_aaveV2_callback_wrongCallerRevert() public {
        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, address(mockPool), uint8(3), uint8(14), sweepCall());
    ...
    function test_unit_lending_flashloans_aaveV2_callback_fuzzInvalidPoolIds(uint8 poolId) public {
        ...
        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, MOOLA, uint8(3), uint8(poolId), sweepCall());
        vm.prank(user);
    ...
    function getAddressFromRegistry() internal {
        MOOLA = chain.getLendingController(Lenders.MOOLA);
    ...
    function populateValidPools() internal {
        validPools.push(PoolCase({poolId: 14, poolAddr: MOOLA, asset: USDC}));
