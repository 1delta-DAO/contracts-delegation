...
contract AaveV2FlashLoanCallbackTest is BaseTest, DeltaErrors {
    ...
    function test_flash_loan_aaveV2_type_granary_pool_with_callbacks() public {
        ...
        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, GRANARY, uint8(3), uint8(7), "");

        vm.prank(user);
        oneDV2.deltaCompose(params);
    }

    function test_flash_loan_aaveV2_type_polter_pool_with_callbacks() public {
        replaceLendingPoolWithMock(POLTER);

        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, POLTER, uint8(3), uint8(11), "");
    ...
    function test_flash_loan_aaveV2_type_wrongCaller_revert() public {
        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, address(mockPool), uint8(3), uint8(7), "");
    ...
    function test_flash_loan_aaveV2_type_fuzz_invalidPoolIds(uint8 poolId) public {
        ...
        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, GRANARY, uint8(3), uint8(poolId), "");
        vm.prank(user);
