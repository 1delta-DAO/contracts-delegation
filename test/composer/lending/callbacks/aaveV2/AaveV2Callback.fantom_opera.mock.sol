...
contract AaveV2FlashLoanCallbackTest is BaseTest, DeltaErrors {
    ...
    function setUp() public virtual {
        string memory chainName = Chains.FANTOM_OPERA;
    ...
    function test_unit_lending_flashloans_aaveV2_callback_granaryPool() public {
        ...
        bytes memory params = CalldataLib.encodeFlashLoan(WFTM, 1e6, GRANARY, uint8(3), uint8(7), sweepCall());
    ...
    function test_unit_lending_flashloans_aaveV2_callback_wrongCallerRevert() public {
        bytes memory params = CalldataLib.encodeFlashLoan(WFTM, 1e6, address(mockPool), uint8(3), uint8(7), sweepCall());
    ...
    function test_unit_lending_flashloans_aaveV2_callback_fuzzInvalidPoolIds(uint8 poolId) public {
        ...
        bytes memory params = CalldataLib.encodeFlashLoan(WFTM, 1e6, GRANARY, uint8(3), uint8(poolId), sweepCall());
        vm.prank(user);
    ...
    function sweepCall() internal returns (bytes memory) {
        return CalldataLib.encodeSweep(WFTM, user, 0, SweepType.VALIDATE);
    }
    ...
    function getAddressFromRegistry() internal {
        ...
        // Get token addresses
        WFTM = chain.getTokenAddress(Tokens.WFTM);
    }

    function populateValidPools() internal {
        validPools.push(PoolCase({poolId: 7, poolAddr: GRANARY, asset: WFTM}));
    }

    function mockERC20FunctionsForAllTokens() internal {
        mockERC20Functions(WFTM);
    }
