...
contract AaveV2FlashLoanCallbackTest is BaseTest, DeltaErrors {
    ...
    function test_flash_loan_aaveV2_type_lendle_pool_with_callbacks() public {
        replaceLendingPoolWithMock(LENDLE);

        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, LENDLE, uint8(3), uint8(1), "");

        vm.prank(user);
        oneDV2.deltaCompose(params);
    }

    function test_flash_loan_aaveV2_type_aurelius_pool_with_callbacks() public {
        replaceLendingPoolWithMock(AURELIUS);

        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, AURELIUS, uint8(3), uint8(2), "");
    ...
    function test_flash_loan_aaveV2_type_wrongCaller_revert() public {
        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, address(mockPool), uint8(3), uint8(1), "");
    ...
    function test_flash_loan_aaveV2_type_fuzz_invalidPoolIds(uint8 poolId) public {
        ...
        bytes memory params = CalldataLib.encodeFlashLoan(USDC, 1e6, LENDLE, uint8(3), uint8(poolId), "");
        vm.prank(user);
