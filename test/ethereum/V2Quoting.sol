 ...
 * revert.
 */
contract EthereumQuotingTest is DeltaSetup {
    address wbpg = 0xc0f14C88250E680eCd70224B7fBa82b7C6560d12;

    function test_ethereum_V2_quote_spot_exact_in_works() external {
    ...
    function test_ethereum_V2_quote_spot_exact_out_works() external {
        address user = testUser;
        vm.assume(user != address(0));
        address assetOut = USDC;
        address assetIn = WETH;
    ...
    function test_ethereum_V3_quote_spot_exact_in() external {
        address user = testUser;
        vm.assume(user != address(0));
        address assetOut = USDC;
    ...
    function test_ethereum_quick_V3_quote_spot_exact_in() external {
        address user = testUser;
        vm.assume(user != address(0));
        address assetOut = USDC;
        address assetIn = USDT;
    ...
    function test_ethereum_sushi_UNI_V2_quote_spot_exact_in() external {
        address user = testUser;
        vm.assume(user != address(0));
    ...
    function test_ethereum_quick_V3_quote_spot_exact_in_dual() external {
        address user = testUser;
        vm.assume(user != address(0));
        address assetOut = USDC;
        address mid = DAI;
    ...
    function getSpotQuotePathDual(address tokenIn, address mid, address tokenOut, uint8 id, uint16 fee) internal view returns (bytes memory data) {
        address pool = testQuoter._v3TypePool(tokenIn, mid, fee, id);
        address pool2 = testQuoter._v2TypePairAddress(mid, tokenOut, UNI_V2);
